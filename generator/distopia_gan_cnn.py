...
from keras.layers import Input, Dense, LeakyReLU, BatchNormalization, Conv1D, Conv3D, Conv3DTranspose, Reshape, Flatten
...
class DistopiaGAN:
    ...
    def __init__(self, data_path):
        ...
        # now setup the gan
        gan_input = Input(shape=(self.noise_dim,1))
    ...
    def load_data(self,path,check=False):
        ...
        with open(path) as infile:
            raw_samples = json.load(infile)
            self.real_samples = [np.array(sample).reshape(self.num_districts,self.blocks_per_district,2,1)/1920 for sample in raw_samples]
            if check == True:
                counter = 0
        ...
        for i in range(num_batches-1):
            self.dataset.append(np.array(self.real_samples[i*self.batch_size:(i+1)*self.batch_size]))
        # add the last separately in case there is not enough
        self.dataset.append(np.array(self.real_samples[(num_batches-1)*self.batch_size:]))


    def construct_layout(self,layout,scalar=1):
        layout_dict = {}
        for d in range(self.num_districts):
            layout_dict[d] = []
            district = layout[d]
            for block in district:
                layout_dict[d].append([block[0][0]*scalar, block[1][0]*scalar]) # layout is nested
        return layout_dict
    ...
    def check_validity(self,layout,scalar=1920):
    ...
    def build_generator(self):
        ...
        self.generator.add(Dense(256, input_shape=(self.noise_dim,1)))
        self.generator.add(BatchNormalization(momentum=0.8))
        self.generator.add(LeakyReLU(alpha=0.2))
        
        assert self.generator.output_shape == (None,8,256)
        self.generator.add(Reshape((2, 2, 2, 256)))

        self.generator.add(Conv3DTranspose(128, kernel_size=(2,1,1), strides=(2,1,1)))
        print(self.generator.output_shape)
        assert self.generator.output_shape == (None, 4, 2, 2, 128)

        self.generator.add(BatchNormalization(momentum=0.8))
        self.generator.add(LeakyReLU(alpha=0.2))
        
        self.generator.add(Conv3DTranspose(64, kernel_size=(1,1,1), strides=(1,1,1)))
        assert self.generator.output_shape == (None, 4, 2, 2, 64)

        self.generator.add(BatchNormalization(momentum=0.8))
        self.generator.add(LeakyReLU(alpha=0.2))
        
        self.generator.add(Conv3DTranspose(1, kernel_size=(2,1,1), strides=(2,1,1), activation='tanh'))
        assert self.generator.output_shape == (None, 8, 2, 2, 1)
    ...
    def build_discriminator(self):
        ...
        self.discriminator.add(Conv3D(32, kernel_size=(2,1,1), input_shape=(self.num_districts,self.blocks_per_district,2,1)))
        self.discriminator.add(LeakyReLU(alpha=0.2))
        self.discriminator.add(Conv3D(16, kernel_size=(4,1,1)))
        self.discriminator.add(LeakyReLU(alpha=0.2))
        self.discriminator.add(Conv3D(8, kernel_size=(2,1,1)))
        self.discriminator.add(LeakyReLU(alpha=0.2))
    ...
    def train_step(self, real_layouts):
        self.discriminator.trainable = False
        noise = np.random.normal(0, 1, size=[self.batch_size, self.noise_dim, 1])
    ...
    def train(self, epochs):
        logfile = open("generator_cnn_log.csv", "w+")
        for epoch in range(epochs):
            ...
            logfile.write("{},{},{}\n".format(dl,gl, ganl))
            self.generator.save("models/generator_{}.h5".format(epoch))
            self.discriminator.save("models/discriminator_{}.h5".format(epoch))
